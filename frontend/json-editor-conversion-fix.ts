...
export function convertV41JSONToEditorFormat(pricingData: V41PricingData): string {
  ...
  pricingData.scopes.forEach((scope, scopeIndex) => {
    ...
    const scopeTotal = scope.role_allocation.reduce((sum, role) => sum + role.cost, 0);
...
export function extractJSONFromContent(content: string): V41PricingData | null {
  console.log('🔍 [JSON-EXTRACT] Extracting JSON from content');
  ...
  try {
    ...
    for (const pattern of jsonPatterns) {
      ...
      if (match) {
        try {
          const jsonStr = match[0].replace(/```json\s*/, '').replace(/\s*```/, '').trim();
          const parsed = JSON.parse(jsonStr);
          
          // Validate that this looks like our V4.1 format
          if (parsed.currency && parsed.scopes && Array.isArray(parsed.scopes)) {
            console.log('✅ [JSON-EXTRACT] Successfully extracted V4.1 JSON:', parsed);
            return parsed as V41PricingData;
          }
        } catch (parseError) {
          console.log('⚠️ [JSON-EXTRACT] JSON parse failed, trying next pattern');
          continue;
        }
      }
    }
...
export function insertPricingToEditor(content: string, onInsert: (formattedContent: string) => void): void {
  ...
  try {
    ...
    if (extractedJSON) {
      ...
      const formattedContent = convertV41JSONToEditorFormat(extractedJSON);
